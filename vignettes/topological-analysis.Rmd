...
```{r}
rob_emp_unif <- robustness_emp(net$net)
rob_emp_dec <- robustness_emp(net$net, ext_seq = "decreasing")
rob_emp_inc <- robustness_emp(net$net, ext_seq = "increasing")
lbm_param <- get_lbm_param(net$net)
...
rob_fun <- lapply(
  ...
  FUN = function(i) {
    A <- simulate_lbm(con = lbm_param$con, 
                      ...
                      nr = lbm_param$nr, 
                      nc = lbm_param$nc)$A
    return(list(unif = robustness_emp(A)$fun,
           dec = robustness_emp(A, ext_seq = "decreasing")$fun,
           inc = robustness_emp(A, ext_seq = "increasing")$fun))
  }
)
```
...
```{r}
bind_cols(rob_fun) %>% 
  mutate(`Primary Extinctions` = seq.int(0, lbm_param$nr)/lbm_param$nr) %>% 
  pivot_longer(cols = contains("..."), names_to = c( "extinction", "index"), 
               names_pattern = "(.*)...(.)", values_to = "Robustness") %>% 
  ggplot(aes(x = `Primary Extinctions`,
             y = Robustness, col = index)) +
...
res_strange %>% 
  ...
  filter(Method %in% c("inc_mc", "inc_lbm")) %>% 
  # mutate(
